...
class _IOS26ButtonState extends State<IOS26Button> {
  ...
  void initState() {
    super.initState();
    _id = _nextId++;
    _channel = MethodChannel('adaptive_liquid_glass/ios26_button_$_id');
  ...
  void didUpdateWidget(IOS26Button oldWidget) {
    ...
    // FIX 25/08 (bug en iOS): el estado enabled del nativo solo se
    // actualizaba si cambiaba la propiedad `enabled` (casi siempre true),
    // IGNORANDO los cambios de onPressed null → callback. Resultado: un
    // botón creado deshabilitado (p.ej. 'Comprobando...' en el detalle de
    // producto) quedaba GRIS para siempre aunque el refresh terminara — el
    // label sí cambiaba pero el nativo nunca se re-habilitaba.
  ...
  double _estimateIntrinsicWidth() {
    if (widget.minSize != null && widget.minSize!.width > 0) {
      return widget.minSize!.width;
    }
    if (widget.sfSymbol != null && widget.label.isEmpty) {
      // Icon only button (square / circular pill)
      return _height;
    }
    if (widget.label.isNotEmpty) {
      // Standard iOS button horizontal padding (~32pt) + char width estimate
      final textWidth = widget.label.length * 9.5;
      final iconWidth = widget.sfSymbol != null ? 24.0 : 0.0;
      final total = 32.0 + textWidth + iconWidth;
      return total.clamp(_height, 400.0);
    }
    // Child mode default fallback
    return 120.0;
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (!kIsWeb && Platform.isIOS) {
      final platformView = UiKitView(
        viewType: 'adaptive_liquid_glass/ios26_button',
      ...
      return LayoutBuilder(
        builder: (context, constraints) {
          final isWidthUnbounded = constraints.maxWidth.isInfinite;
          final double? appliedWidth = widget.minSize != null
              ? widget.minSize!.width
              : (isWidthUnbounded ? _estimateIntrinsicWidth() : null);
