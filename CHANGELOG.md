...
* **FIX**: `IOS26NativeToolbar` fallback usa `_fallbackHeroTag` único + `transitionBetweenRoutes: false` — elimina "multiple heroes share the same tag" con shell + página interna (portado de Ledexsoft).
* **FIX**: `ios26_scaffold` aplica el margen de 62px del back solo en iOS (no en macOS, donde cualquier ventana es windowed y el botón quedaba separado) (portado de Ledexsoft).
* **FEAT**: `iOS26NativeTabBarManager` modernizado a iOS 18+ (`UITab`/`UISearchTab`, `enableModernTabBar`) con soporte legacy iOS 14-17 (portado de Ledexsoft, sin el código app-específico).

## [0.1.114]
* **NEW**: `AdaptiveBlurView` now uses the native AppKit path on macOS (`NSVisualEffectView` on macOS <26, `NSGlassEffectView` on macOS 26+) via `AppKitView`, matching the existing Liquid Glass look of iOS 26+ instead of the Flutter fallback.
* **NEW**: Added `AdaptiveDivider` (hairline on iOS/macOS, Material 3 `Divider` on Android).
* **NEW**: Added `AdaptiveSearchBar` (`CupertinoSearchTextField` on iOS/macOS, Material rounded `TextField` on Android, with clear action).
* **NEW**: Added `AdaptiveSwitchListTile` (`CupertinoSwitch` list tile on iOS/macOS, Material `ListTile` + `Switch` on Android).

## [0.1.113]
* **FIX**: Fixed CALayer crash (NaN/infinite width coordinates) when `AdaptiveButton` / `IOS26Button` is used naked in a `Row` or unconstrained flex layout. Added automatic intrinsic width estimation (`_estimateIntrinsicWidth`) and `LayoutBuilder` safety.
* **FIX**: Decoupled `PlatformInfo.isMacOS` from `PlatformInfo.isIOS26OrHigher()`, allowing macOS tests and builds to avoid invalid `UiKitView` instantiation. Added `PlatformInfo.isGlassSupported`.
* **NEW**: Added `debugPlatformOverride`, `debugIOSVersionOverride`, and `resetDebugOverrides()` to `PlatformInfo` for reliable unit/widget testing across platforms.
* **NEW**: Added `AdaptiveProgressIndicator` with circular and linear modes (`CupertinoActivityIndicator` on iOS/macOS vs Material 3 `CircularProgressIndicator` / `LinearProgressIndicator` on Android).
* **NEW**: Added `AdaptiveCard.glass` constructor with `AdaptiveBlurView` on Apple platforms and elevated surface container styling on Android, plus `onTap` interaction support.
* **NEW**: Added `showAdaptiveActionSheet` and `AdaptiveActionSheetAction` with native `CupertinoActionSheet` on iOS/macOS and Material 3 modal bottom sheet on Android.
* **FIX**: Ensured `AdaptiveListTile` wraps the Material `ListTile` with `Material(type: MaterialType.transparency)` so it never throws in non-Material trees.

## [0.1.112]
* **NEW**: Added native Liquid Glass category grid (`iOS26GlassGridPlatformView.swift` / `ios26_glass_grid.dart`) using `UIGlassContainerEffect` for fusion and light spill.
* **FIX**: Fixed double padding on automatic back button in `IOS26NativeToolbar` (`leadingLeft: 0`).
...
* Centralized iOS Liquid Glass material creation with safe iOS 15 fallback.
* Updated native blur and toolbar surfaces to use `UIGlassEffect` and
  `UIGlassContainerEffect` on iOS 26+.
* Let standard iOS 26 tab bars use Apple's automatic Liquid Glass appearance.
* Fixed the iOS example deployment target and Swift Package Manager
