...
A Flutter plugin (`adaptive_liquid_glass`) that renders platform-adaptive widgets:
...
The iOS native code lives under `ios/adaptive_liquid_glass/Sources/adaptive_liquid_glass/` (Swift). The Dart plugin code lives under `lib/`.
...
1. **Dart side** (`lib/src/widgets/ios26/ios26_*.dart`): Creates a `UiKitView` with `viewType: 'adaptive_liquid_glass/ios26_{component}'`, passes configuration as `creationParams`, and sets up a `MethodChannel` with pattern `adaptive_liquid_glass/{component}_{instanceId}` for callbacks.

2. **Swift factory** (`ios/adaptive_liquid_glass/Sources/adaptive_liquid_glass/iOS26*View.swift`): Implements `FlutterPlatformViewFactory`, registered in `AdaptivePlatformUiPlugin.swift`.
...
`ios/adaptive_liquid_glass/Sources/adaptive_liquid_glass/AdaptivePlatformUiPlugin.swift` registers all platform view factories. Android plugin (`android/`) is a no-op stub since Material widgets are pure Dart.
...
Located in `ios/adaptive_liquid_glass/Sources/adaptive_liquid_glass/`:
