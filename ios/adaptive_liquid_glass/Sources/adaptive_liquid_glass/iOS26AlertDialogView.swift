...
class iOS26AlertDialogView: NSObject, FlutterPlatformView {
    ...
    init(frame: CGRect, viewId: Int64, args: Any?, messenger: FlutterBinaryMessenger) {
        self.channel = FlutterMethodChannel(name: "adaptive_liquid_glass/ios26_alert_dialog_\(viewId)", binaryMessenger: messenger)
    ...
    private func setupAlert(
        ...
        if #available(iOS 15.0, *) {
            ...
            // iOS 26+: real Liquid Glass (refraction + adaptive light spill)
            // iOS <26: classic system material blur
            let blurView: UIVisualEffectView
            if #available(iOS 26.0, *) {
                let glass = UIGlassEffect(style: .regular)
                glass.isInteractive = true
                blurView = UIVisualEffectView(effect: glass)
            } else {
