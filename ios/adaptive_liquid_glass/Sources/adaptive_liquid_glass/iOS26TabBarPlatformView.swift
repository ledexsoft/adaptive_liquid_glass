...
class iOS26TabBarPlatformView: NSObject, FlutterPlatformView, UITabBarDelegate {
    ...
    init(frame: CGRect, viewId: Int64, args: Any?, messenger: FlutterBinaryMessenger) {
        self.channel = FlutterMethodChannel(
            name: "adaptive_liquid_glass/ios26_tab_bar_\(viewId)",
        ...
        if let dict = args as? [String: Any] {
            ...
            hideLabels = (dict["hideLabels"] as? Bool) ?? false
        }
        ...
        func buildItems(_ range: Range<Int>) -> [UITabBarItem] {
            ...
            for i in range {
                let realTitle = (i < labels.count) ? labels[i] : nil
                let title = hideLabels ? "" : realTitle
                ...
                // Accesibilidad: el label real vive en accessibilityLabel
                // cuando se ocultan los títulos (solo iconos).
                if hideLabels, let realTitle = realTitle {
                    item.accessibilityLabel = realTitle
                }
